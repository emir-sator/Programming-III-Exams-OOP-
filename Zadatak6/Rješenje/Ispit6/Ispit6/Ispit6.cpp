	...
	ZavrsniRad* ZakaziOdbranuRada(string brojIndexa, string datum) {
		for (size_t i = 0; i < _teme.getTrenutno(); i++)
		{
			if (_teme.getElement1(i) == brojIndexa) {
				...
				float suma = 0;
				for (vector<Poglavlje>::iterator j = _teme.getElement2(i).GetPoglavlja().begin();
					j != _teme.getElement2(i).GetPoglavlja().end(); j++)
				{
...
int main() {
	...
	/*u zavrsni rad dodaje novo poglavlje i njegov sadrzaj. ukoliko poglavlje vec postoji u zavrsnom radu,ž
	...
	zr1 = nastavnici[0]->ZakaziOdbranuRada("IB130011", "25.09.2018");//student sa brojem indeksa IB130011 jos uvijek nije prijavio rad
	...
	i tom prilikom ostvarili ocjenu vecu od proslijedjene, u zasebnom thread.u, posalje email poruku (mail adresa: brojIndeksa@edu.fit.ba) sa sadrzajem, "Postovani {brojIndeksa}, 
	uzimajuci u obzir cinjenicu da ste kod mentora {imePrezimeMentora} uspjesno odbranili rad sa ocjenom {ocjena} cast nam je pozvati vas na dodjelu nagrada za najbolje studente 
	koja ce se odrzatu u na FIT-u 20.09.2018. godine."
