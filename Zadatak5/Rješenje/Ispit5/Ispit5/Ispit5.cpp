...
const char* statusTxt[] = { "ZAHTJEV_ZAPRIMLJEN", "ODUSTAO", "ODBIJENA", "IZDATA", "ISTEKLA", "PONISTENA" };
enum Sortiranje { RASTUCI, OPADAJUCI };
enum Drzava { SAD, AUSTRALIJA, ENGLESKA };
const char* drzavaTxt[] = { "SAD", "AUSTRALIJA", "ENGLESKA" };
			...
			while (promjena)
			{
				...
				for (size_t i = 0; i < _trenutno - 1; i++)
				{
					if (*_elementi1[i] > * _elementi1[i + 1]) {
						...
						swap(_elementi2[i], _elementi1[i + 1]);
						promjena = true;
					}
				}
			}
...
protected:
	...
	Datum _vazenjeDO;
	char* _napomena;
	// string se odnosi na razlog promjene statusa,
	//a moguće je samo 10 izmjena statusa
...
public:
	...
	friend ostream& operator << (ostream& COUT, PoslovnaViza& obj) {
		COUT << drzavaTxt[obj._drzava] << " " << obj._vazenjeOD << " " << obj._vazenjeDO << " " << obj._kompanija << endl;
...
public:
	...
	friend ostream& operator << (ostream& COUT, TuristickaViza& obj) {
		COUT << drzavaTxt[obj._drzava] << " " << obj._vazenjeOD << " " << obj._vazenjeDO << " " << obj._adresa << endl;
		COUT << ((obj._napomena != nullptr) ? obj._napomena : "") << endl;
		COUT << "STATUSI -> " << obj._statusi << endl;
		return COUT;
...
(POJAM VAŽEĆA PODRAZUMIJEVA DA JE POSLJEDNJI STATUS VIZE BIO IZDATA TJ. DA VIZA NIJE PONIŠTENA ILI ISTEKLA)
VIZA ZA TU DRŽAVU, BEZ OBZIRA NA NJENU VRSTU.
- JE NEKA OD RANIJIH VIZA PONIŠTENA (OVO SE ODNOSI SAMO NA VIZE ZA DRŽAVU: SAD)
*/
	bool DodajVizu(Viza* v) {
		if (v->GetVazenjeOD().GetYear() - _datumRodjenja->GetYear() < 18) {
			return false;
		}
		for (size_t i = 0; i < _vize.size(); i++)
		{
		...
		if (dynamic_cast<TuristickaViza*>(v) != nullptr) {
