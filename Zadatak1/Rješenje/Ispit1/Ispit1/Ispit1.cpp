...
class Izuzetak : public exception {
	string _funkcija;
public:
	Izuzetak(const char* poruka, string funckija) :exception(poruka), _funkcija(funckija) {
...
public:
	...
	bool AddElement(const T1& prvi, const T2& drugi) {
	...
	void RemoveElement(const T1& el) {
		for (size_t i = 0; i < _trenutnoElemenata; i++)
		{
			if (_elementi1[i] == el) {
				...
				{
					if (i != j) {
						temp1[brojac] = _elementi1[j];
						temp2[brojac] = _elementi2[j];
						brojac++;
					}
				}
				delete[]_elementi1;
...
class Dogadjaj
{
	...
	bool ProvjeriValidnost(string s) {
		return regex_search(s, regex("UBITI|MRZITI|ubiti|mrziti|RE|NC|TM"));
	}
...
public:
	...
	Dogadjaj& operator=(const Dogadjaj& d) {
		if (this != &d) {
			...
			_naziv = new char[strlen(d._naziv) + 1];
			strcpy_s(_naziv, strlen(d._naziv) + 1, d._naziv);
			...
			for (vector<Dogadjaj>::iterator j = i->GetDogadjaji().begin(); j != i->GetDogadjaji().end(); j++)
			{
				if (j->GetDatumObaveze() > d) {
					...
					thread t(
						...
						"Dogadjaj " + j->GetNaziv() + " je zakazan za " + to_string(j->GetDatumObaveze().GetDani() - d.GetDani()) + " dana" + ", " +
...
#pragma region Dogadjaj
	...
	if (ispitPRIII.AddObavezu("Preraditi pdf materijale"))cout << "Obaveza1 dodana!" << endl;
	//onemoguciti dupliranje obaveza
	if (!ispitPRIII.AddObavezu("Preraditi pdf materijale"))cout << "Obaveza nije dodana!" << endl;
	if (!ispitPRIII.AddObavezu("Pregledati video materijale"))cout << "Obaveza nije dodana!" << endl;
	if (ispitPRIII.AddObavezu("Preraditi ispitne zadatke"))cout << "Obaveza2 dodana!" << endl;
	if (ispitPRIII.AddObavezu("Samostalno vjezbati"))cout << "Obaveza3 dodana!" << endl;

	if (ispitBPII.AddObavezu("Preraditi knjigu SQL za 24 h"))cout << "Obaveza4 dodana!" << endl;
	if (ispitBPII.AddObavezu("Pregledati video materijale"))cout << "Obaveza5 dodana!" << endl;
	if (ispitBPII.AddObavezu("Napraviti bazu za konkretnu aplikaciju"))cout << "Obaveza6 dodana!" << endl;
